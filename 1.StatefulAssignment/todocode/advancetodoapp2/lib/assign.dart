...
Future<void> updateData(ToDoModelClass obj) async {
  final localDB = await database;
  await localDB
      .update("app", obj.toMap(), where: 'app=?', whereArgs: [obj.idNo]);
}

Future<List<ToDoModelClass>> getData() async {
  final localDB = await database;

  List<Map<String, dynamic>> getMap = await localDB.query("app");
...
// List<ToDoModelClass> getDataList(){
//   return List.generate(getData().length, (index){
//   });
// }
...
class _TODOAppUIState extends State<TODOAppUI> {
  ...
  // List<ToDoModelClass> todoList= getData() as List<ToDoModelClass>;
